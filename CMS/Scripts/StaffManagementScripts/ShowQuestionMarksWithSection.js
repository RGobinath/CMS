$(function () {
    ...
    var subgrid_selector1 = "#SurveyMarkssecListJqgrid";
    var subpager_selector1 = "#SurveyMarkssecListPager";
    //resize to fit page size
    $(window).on('resize.jqGrid', function () {
        $(subgrid_selector1).jqGrid('setGridWidth', $('#grid').width());
    })
    //resize on sidebar collapse/expand 
    var parent_column = $(subgrid_selector1).closest('[class*="col-"]');
    $(document).on('settings.ace.jqGrid', function (ev, event_name, collapsed) {
        if (event_name === 'sidebar_collapsed' || event_name === 'main_container_fixed') {
            //setTimeout is for webkit only to give time for DOM changes and then redraw!!!
            setTimeout(function () {
                $(subgrid_selector1).jqGrid('setGridWidth', parent_column.width());
            }, 0);
        }
    })
    ...
    $(document).on('ajaxloadstart', function (e) {
        $(subgrid_selector1).jqGrid('GridUnload');
        $('.ui-jqdialog').remove();
    });


    $(subgrid_selector1).jqGrid({
        datatype: 'local',
        type: 'GET',
        colNames: ['Id', 'Academic Year', 'Campus', 'Grade', 'Survey Number', 'Survey Date', 'Staff Name', 'No. of Students Attended', 'Question in Survey', 'Score in Survey', 'Average Score by Question'],
        ...
        loadComplete: function () {
            var table = this;
            $(subgrid_selector1).footerData('set', { StudentSurveyQuestion: 'Total:' });
            //var colScore = $(subgrid_selector1).jqGrid('getCol', 'Score', false, 'sum');
            //$(subgrid_selector1).footerData('set', { Score: colScore });
            //var colAvg = $(subgrid_selector1).jqGrid('getCol', 'Average', false, 'sum');
            ////colAvg = colAvg / $("#StudentCount").val();
            //$(subgrid_selector1).footerData('set', { Average: colAvg });
            setTimeout(function () {
    ...
    $(subgrid_selector1).navGrid(subpager_selector1, { add: false, edit: false, del: false, search: false, refresh: true, refreshicon: 'ace-icon fa fa-refresh green' });

    $(subgrid_selector1).jqGrid('navButtonAdd', subpager_selector1, {
