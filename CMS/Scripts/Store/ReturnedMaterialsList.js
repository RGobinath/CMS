$(function () {
    ...
    jQuery(grid_selector).jqGrid({
        url: '/Store/ReturnedMaterialJqGrid?AcceptedStatus=' + "Open",
        datatype: 'json',
        mtype: 'POST',
        height: '220',
    ...
    $("#AcceptedStatus").change(function () {
        $(grid_selector).setGridParam(
                {
                    datatype: "json",
                    url: '/Store/ReturnedMaterialJqGrid',
                    postData: { AcceptedStatus: $("#AcceptedStatus").val() },
                    page: 1
                }).trigger("reloadGrid");
    });
...
function formateadorLink(cellvalue, options, rowObject) {
    if (rowObject[8] == "Open") {
        return "<a href=/Store/AcceptReturnedMaterials?MatRetId=" + rowObject[0] + ">" + cellvalue + "</a>";
    }
    else {
