var MatINNgrid_selector = "#MaterialSkuList";
var MatINNpager_selector = "#MaterialSkuListPager";

$(function () {
    //resize to fit page size
    $(window).on('resize.jqGrid', function () {
        $(MatINNgrid_selector).jqGrid('setGridWidth', $(".page-content").width());
    })
    //resize on sidebar collapse/expand
    var parent_column = $(MatINNgrid_selector).closest('[class*="col-"]');
    $(document).on('settings.ace.jqGrid', function (ev, event_name, collapsed) {
        if (event_name === 'sidebar_collapsed' || event_name === 'main_container_fixed') {
            //setTimeout is for webkit only to give time for DOM changes and then redraw!!!
            setTimeout(function () {
                $(MatINNgrid_selector).jqGrid('setGridWidth', parent_column.width());
            }, 0);
        }
    })
...
//    var lastsel2;
    jQuery(MatINNgrid_selector).jqGrid({
        url: '/Store/MaterialSkuListJqGrid?Id=' + Id + '&Store=' + $("#ddlStore").val(),
        datatype: 'json',
        mtype: 'POST',
        colNames: ['SKU Id', 'Material Ref Id', 'Materials', 'Material Group', 'Material Sub Group', 'Ord.Units', 'Rcvd.Units', 'Old Prices', 'Ord.Qty', 'Rcvd.Qty', 'Dmg.Qty', 'Unit Price', 'Tax in %', 'Discount in %', 'Total Price', 'Dmg.Desc / Remarks', ''],
        colModel: [
              ...
              { name: 'Tax', index: 'Tax', width: 90, editable: true, editoptions: {
                  dataInit: function (element) {
                      $(element).keyup(function () {
                          var rowId = parseInt($(this).attr("id"));
                          CalculateTotalPrice(rowId);
              ...
              { name: 'Discount', index: 'Discount', width: 110, editable: true, editoptions: {
                  dataInit: function (element) {
                      $(element).keyup(function () {
                          var rowId = parseInt($(this).attr("id"));
                          CalculateTotalPrice(rowId);
        ...
        gridComplete: function () {
            var rdata = $(MatINNgrid_selector).getRowData();
            if (rdata.length > 0) {
    ...
    jQuery(MatINNgrid_selector).jqGrid('navGrid', MatINNpager_selector, {
        //navbar options
        edit: false,
...
function CalculateTotalPrice(rowId) {
    ...
    $("#" + rowId + "_TotalPrice").val(($("#" + rowId + "_UnitPrice").val() - ($("#" + rowId + "_UnitPrice").val() * ($("#" + rowId + "_Discount").val() / 100)) + ($("#" + rowId + "_UnitPrice").val() * ($("#" + rowId + "_Tax").val() / 100))) * $("#" + rowId + "_ReceivedQty").val());
}
...
function skuValidation() {
    ...
    if (Id == 0) {
        ...
        var skuLst1 = '';
        var Id = $("#Id").val();
        var dataIds = $(MatINNgrid_selector).jqGrid('getDataIDs');
        if (dataIds == "") {
        ...
        for (var i = 0, list = dataIds.length; i < list; i++) {
            ...
            if (dmgqty != "") {
                if (parseInt(dmgqty) != 0 && dmgdesc == "") {
                    ErrMsg("Please type Damage description", function () {
                        ...
                        jQuery(MatINNgrid_selector).editRow(dataIds[i], true);
                    });
                    return false;
                    break;
                }
            ...
            jQuery(MatINNgrid_selector).saveRow(dataIds[i], true);
            jQuery(MatINNgrid_selector).editRow(dataIds[i], true);
        }
        $(MatINNgrid_selector).setGridParam({ url: '/Store/MaterialSkuListJqGrid?Id=' + Id }).trigger("reloadGrid");
        return true;
    }
}
