...
$(function () {
    ...
    var grid_selector = "#StaffIssueManagementDateWiseReportList";
    var pager_selector = "#StaffIssueManagementDateWiseReportListPager";
    $(grid_selector).jqGrid({
        url: '/StaffIssues/StaffIssueManagementDateWiseReportListJqGrid',
        datatype: 'json',
        mtype: 'GET',
        ...
        colModel: [
                    ...
                    { name: 'TotalCount', index: 'TotalCount', align: 'center' }
        ],
    ...
    if ($("#showexcel").val() == "True") {
        jQuery(grid_selector).jqGrid('navButtonAdd', pager_selector, {
            ...
            onClickButton: function () {
                window.open("StaffIssueManagementDateWiseReportListJqGrid" + '?Performer=' + $("#ddlPerformer").val() + '&FromDate=' + $("#txtFromDate").val() + '&ToDate=' + $("#txtToDate").val() + '&Campus=' + $("#ddlCampus").val() + '&IssueType=' + $("#ddlIssueType").val() + '&IssueGroup=' + $("#ddlIssueGroup").val() + '&rows=9999999' + '&ExptXl=1');
            }
        });
    }
    jQuery(grid_selector).jqGrid('navButtonAdd', pager_selector, {
        ...
        onClickButton: function () {
            window.open("StaffIssueManagementDateWiseCountReportPDF" + '?Performer=' + $("#ddlPerformer").val() + '&FromDate=' + $("#txtFromDate").val() + '&ToDate=' + $("#txtToDate").val() + '&Campus=' + $("#ddlCampus").val() + '&IssueType=' + $("#ddlIssueType").val() + '&IssueGroup=' + $("#ddlIssueGroup").val() + '&rows=9999999');
        }
    });
    $("#btnReset").click(function () {
        ...
        jQuery(grid_selector).setGridParam(
       {
           datatype: "json",
           url: '/StaffIssues/StaffIssueManagementDateWiseReportListJqGrid',
           postData: { Performer: $("#viewbagPerformer").val(), FromDate: "", ToDate: "", Campus: "", IssueGroup: $("#viewbagDeptCode").val(), IssueType: "" },
           page: 1
       }).trigger("reloadGrid");
        if ($("#ddlPerformer").val() == "") {
    ...
    $("#btnSearch").click(function () {
        ...
        jQuery(grid_selector).setGridParam(
       {
           datatype: "json",
           url: '/StaffIssues/StaffIssueManagementDateWiseReportListJqGrid',
           postData: { Performer: Performer, FromDate: FromDate, ToDate: ToDate, Campus: Campus, IssueGroup: IssueGroup, IssueType: IssueType },
           page: 1
       }).trigger("reloadGrid");
        if ($("#ddlPerformer").val() != "") {
            $("#StaffIssueMgmtDateWiseRptChart1").hide();
            loadchart();
        }
        if ($("#ddlPerformer").val() == "") {
            $("#StaffIssueMgmtRptChart1").show();
...
function ShowCallDetails1(HeaderName, Performer) {
    ModifiedLoadPopupDynamicaly("/StaffIssues/ShowCallDetails1?HeaderName=" + HeaderName + '&Performer=' + Performer, $('#ShowCallDetails1'),
...
function loadchart() {
    ...
    $.ajax({
        type: 'Get',
        async: false,
        url: '/StaffIssues/GetStaffIssueManagementDateWiseReportChart?Performer=' + $("#ddlPerformer").val() + '&FromDate=' + $("#txtFromDate").val() + '&ToDate=' + $("#txtToDate").val() + '&Campus=' + $("#ddlCampus").val() + '&IssueGroup=' + $("#ddlIssueGroup").val() + '&IssueType=' + $("#ddlIssueType").val(),
        success: function (ReportCount) {
            ...
            //}
            $.plot($("#StaffIssueMgmtDateWiseRptChart"), data_pie, {
                series: {
                    pie: {
                        show: true,
                        innerRadius: 0.5,
