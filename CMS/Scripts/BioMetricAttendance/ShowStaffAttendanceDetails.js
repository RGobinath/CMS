jQuery(function () {
   ...
   // alert(MonthYear);
    ...
    $(window).on('resize.jqGrid', function () {
        $("#StaffDailyAttendanceIOSummaryGrid").jqGrid('setGridWidth', $(".page-content").width() - 290);
    });
    //resize on sidebar collapse/expand
    var parent_column = $("#StaffDailyAttendanceIOSummaryGrid").closest('[class*="col-"]');
    $(document).on('settings.ace.jqGrid', function (ev, event_name, collapsed) {
        if (event_name === 'sidebar_collapsed' || event_name === 'main_container_fixed') {
            //setTimeout is for webkit only to give time for DOM changes and then redraw!!!
            setTimeout(function () {
                $("#StaffDailyAttendanceIOSummaryGrid").jqGrid('setGridWidth', parent_column.width());
            }, 0);
        }
    });
    jQuery("#StaffDailyAttendanceIOSummaryGrid").jqGrid({
        url: '/BioMetricAttendance/StaffDailyAttendanceIOSummaryJqGrid?AttendanceFromDate=' + PopUpAttendanceFromDate + '&AttendanceToDate=' + PopUpAttendanceToDate + '&IdNumber=' + PopUpIdNumber ,
        datatype: 'json',
        type: 'GET',
        ...
        rowNum: 5000,
        rowList: [5000],
        pager: "#StaffDailyAttendanceIOSummaryGridPager",
        sortname: 'Id',
        sortorder: "Desc",
        height: 400,
    ...
    jQuery("#StaffDailyAttendanceIOSummaryGrid").jqGrid('navGrid', "#StaffDailyAttendanceIOSummaryGridPager",
        { 	//navbar options
            edit: false,
    ...
    $("#StaffDailyAttendanceIOSummaryGrid").jqGrid('navButtonAdd', "#StaffDailyAttendanceIOSummaryGridPager", {
        ...
        onClickButton: function () {
            window.open("StaffDailyAttendanceIOSummaryJqGrid" + '?rows=9999' +
                        '&page=1' +
                        '&sidx=Name' +
                        '&sord=desc' +
