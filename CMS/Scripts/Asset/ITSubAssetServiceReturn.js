...
function FillStaffGroupByCampus() {
    ...
    //    if (Campus != "" && Campus != null) {
    $.getJSON("/Base/StaffGroupByCampus?Campus=",
      function (fillbc) {
          ddlbc.empty();
          ddlbc.append($('<option/>', { value: "", text: "Select" }));
          $.each(fillbc, function (index, itemdata) {
...
function FillDepatmentByCampusandGroup() {
    ...
    if (StaffGroup != "" && StaffGroup != null) {
        $.getJSON("/Base/DepatmentByCampusandGroup?Campus=" + '&GroupName=' + StaffGroup,
          function (fillbc) {
              ddlbc.empty();
...
function FillStaffNameByCampusWithGroupandDepartment() {
    ...
    if (StaffGroup != "" && StaffGroup != null) {
        $.getJSON("/Base/StaffNameByCampusWithGroupandDepartment?Campus=" + '&GroupName=' + StaffGroup + '&Department=' + Department,
          function (fillbc) {
              ddlbc.empty();
              ddlbc.append($('<option/>', { value: "", text: "Select" }));
              $.each(fillbc, function (index, itemdata) {
...
jQuery(function ($) {
    ...
    $('#btnAddAsset').click(function () {
        ...
        if (AssetUserType == "Student") {
            ...
            if ($("#IsSubAsset").val() == "True" && $("#txtITAssetCode").val() == "" && UserType != "Not Applicable") {
                $("#hdnAssetDet_Id").val('');
        ...
        if (InvoiceDetailsId == 0 || VendorName == "" || UserType == "" || Amount == "") {
            ErrMsg("Please Fill Required Fields");
            return false;
        }
        $.ajax({
            ...
            success: function (data) {
                if (data == true) {
                    ...
                    window.location.href = "/Asset/ITSubAssetService";
                    return true;
...
function reloadGrid() {
    $('#grid-table').setGridParam(
         {
             datatype: "json",
             url: '/Asset/ITSubAssetServicejqGrid',
