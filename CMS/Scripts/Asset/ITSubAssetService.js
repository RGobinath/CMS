...
function TaskComplete(cellvalue, options, rowObject) {
    if (rowObject[7] == "Service") {
        return "<input type='button' value='Move Back' id='SubServiceReturn' style='background-color:#8B0000; border: none;color:white;padding: 5px 25px;text-align:center;font-size: 10px;border-radius:5px;' onclick=MoveToBack('" + rowObject[0] + "');\>";
    }
    var empty = "";
    return empty;
}
function MoveToBack(AssetId) {
    ModifiedLoadPopupDynamicaly("/Asset/ITSubAssetServiceReturn?AssetId=" + AssetId, $('#MoveToBack1'), function () {
