    ...
    */
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            slotCnt = 0;
            var PeriodNum = 1;// Declared by Gobi
            while (slotTime < maxTime) {
                slotDate = t.start.clone().time(slotTime); // will be in UTC but that's good. to avoid DST issues
                minutes = slotDate.minutes();
                if (PeriodNum < 6) {
                    var periodTime = "";
                    periodTime =PeriodNum.toString()+" "+ formatDate(slotDate, opt('axisFormat')) ;
                    //alert(formatDate(slotDate, opt('axisFormat'))); Display the time duration like 10am, 11am.. Gobi
                    s +=
                        "<tr class='fc-slot" + slotCnt + ' ' + (!minutes ? '' : 'fc-minor') + "'>" +
                        "<th class='fc-agenda-axis " + headerClass + "'>" +
                        //((!slotNormal || !minutes) ? htmlEscape(formatDate(slotDate, opt('axisFormat'))) : '&nbsp;')+
                        ((!slotNormal || !minutes) ? htmlEscape(PeriodNum) : '&nbsp;') +// Assign the period number to the left side of the time table
                        "</th>" +
                    ...
                    s +=
                        ...
                        "<th class='fc-agenda-axis " + headerClass + "'>" +
                        //((!slotNormal || !minutes) ? htmlEscape(formatDate(slotDate, opt('axisFormat'))) : '&nbsp;')+
                        ((!slotNormal || !minutes) ? htmlEscape("Break") : '&nbsp;') +// Assign the period number to the left side of the time table
                        "</th>" +
                        "<td class='" + contentClass + "'>" +
                        "<div style='position:relative'>&nbsp;</div>" +
                        "</td>" +
                ...
                slotCnt++;
                PeriodNum++;// Increment the period number
            }
        ...
        function buildDayTableBodyHTML() {
            ...
            for (col = 0; col < colCnt; col++) {
                ...
                date = cellToDate(0, col);
                //alert(dayIDs[date.day()]); For Top Headers //Gobi
                classNames = [
                    'fc-col' + col,
                    'fc-' + dayIDs[date.day()],
                    contentClass
                ...
                if (date.isSame(today, 'day')) {
                    classNames.push(
                        //tm + '-state-highlight',// Hidden by Gobi for current date high lighting
                        tm,
    ...
    function AgendaEventRenderer() {
        ...
        function slotSegHtml(event, seg) {
            ...
            //alert(t.getEventTimeText(event)); To display the time slot in the calendar like "12:00 - 1:00" Gobi
            html +=
                " class='" + classes.join(' ') + "'" +
                " style=" +
                    "'" +
                ...
                //htmlEscape(t.getEventTimeText(event)) + Display the event time in the calendar like "12:00 - 1:00" Gobi
                //"</div>" +
            ...
            if (seg.isEnd && isEventResizable(event)) {
                html +=
                    //"<div class='ui-resizable-handle ui-resizable-s'>=</div>"; Display the "=" for resize the event in the calendar Gobi
