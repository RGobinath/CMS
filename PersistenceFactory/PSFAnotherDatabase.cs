...
{
    public class PSFAnotherDatabase : IDisposable
    {
        #region Properties
        ...
        public PSFAnotherDatabase(List<String> assemblyName,string connectionString)
        {
            try
            {
                        ...
                        if (searchCriteria != null)
                        {
                            if (searchCriteria.Count > 0)
                            {
                                ...
                                foreach (KeyValuePair<string, object> keyValuePair in searchCriteria.Where(x => x.Value != null))
                                {
                                    ...
                                    else if (keyValuePair.Value.GetType() == typeof(Int32[]))
                                    {
                                        criteria.Add(Expression.In(keyValuePair.Key, (Int32[])keyValuePair.Value));
                                    }
                                }
                            }
                        }
        ...
        #region Added By Gobi For TransportManagement Toolbar search
        ...
        ///Added by Micheal for Not EQ search
        ///

        public Dictionary<long, IList<T>> GetListWithExactSearchCriteriaCountWithNotEqualProperty<T>(int? page, int? pageSize, string sortType, string sortBy, Dictionary<string, object> searchCriteria, string ColumnName, string[] values)
        {
            try
            {
                Dictionary<long, IList<T>> ListWithCount = new Dictionary<long, IList<T>>();
                            ...
                            if (searchCriteria.Count > 0)
                            {
                                foreach (KeyValuePair<string, object> keyValuePair in searchCriteria.Where(x => x.Value != null))
                                {
                                    if (keyValuePair.Value.GetType() == typeof(string))
                                    {
                                        //criteria.Add(Expression.Not(Expression.Like(keyValuePair.Key.ToString(), sPercentage + keyValuePair.Value + sPercentage).IgnoreCase()));
                                         //criteria.Add(Expression.NotEqProperty(keyValuePair.Key.ToString(),  keyValuePair.Value.ToString()));
                                        criteria.Add(Expression.Not(Expression.Eq(keyValuePair.Key.ToString(), keyValuePair.Value.ToString())));
                                    }
                                    else if (keyValuePair.Value.GetType().IsPrimitive)
                                    {
                                        criteria.Add(Expression.Not(Expression.Eq(keyValuePair.Key, keyValuePair.Value)));
